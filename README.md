...
祥子的书 是一个面向个人使用场景的 TXT 在线阅读器，支持上传本地 TXT 文件、自动解析章节、管理书架、同步阅读进度，以及按自己的文本格式定制章节识别规则。
...
本项目已配置为 PWA（Progressive Web App），应用名称为 **「祥子的书」**，支持安装到安卓 / iOS 设备桌面，体验接近原生 App。

### 安卓（Chrome）

1. 用手机 Chrome 访问前端地址（如 `http://你的IP:21413`）
2. 点击地址栏右侧 **⋮** → **"添加到主屏幕"**
3. 确认安装，桌面会出现「祥子的书」图标
...
项目已完整配置 PWA，应用名称「祥子的书」，支持安装到安卓 / iOS 桌面。
